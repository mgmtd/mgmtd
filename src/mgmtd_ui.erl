...
%% and call `http_get/2` / `http_post/2` from their callbacks.
%% @end
%%%-------------------------------------------------------------------
-module(mgmtd_ui).

-export([http_get/2, http_post/2, compile/0, cowboy_routes/0, cowboy_routes/1]).

-define(UI_BASE, <<"/mgmtd/ui">>).
-define(CSS, <<"/mgmtd/ui/static/mgmtd_ui.css">>).
-define(HTML, <<"text/html; charset=utf-8">>).

-spec http_get(index | content, cowboy_req:req()) -> cowboy_req:req().
http_get(Kind, Req) ->
    Qs = cowboy_req:parse_qs(Req),
    Path = qs_val(Qs, <<"path">>),
    Mods = modules(),
    Mode = case lists:keyfind(<<"mode">>, 1, Qs) of
               {_, V} -> parse_mode(V);
               false -> infer_mode(Path, Mods)
           end,
    Body = page(Kind, Path, #{mode => Mode, mods => Mods}),
...
cowboy_routes() ->
    cowboy_routes(mgmtd_ui_handler).

-spec cowboy_routes(module()) -> [{string(), module(), term()}].
cowboy_routes(Handler) ->
    [{"/mgmtd/ui/static/[...]", cowboy_static, {priv_dir, mgmtd, "ui"}},
     {"/mgmtd/ui/content", Handler, content},
     {"/mgmtd/ui/content/", Handler, content},
...
compile() ->
    case code:priv_dir(mgmtd) of
        ...
        Priv ->
            ...
            compile_files(
              ["mgmtd_ui_input.dtl",
               "mgmtd_ui_tree_node.dtl",
               "mgmtd_ui_kv_row.dtl",
               "mgmtd_ui_leaf.dtl",
               "mgmtd_ui_container.dtl",
               "mgmtd_ui_nested.dtl",
...
save(Qs) ->
    ...
    case mgmtd_restconf_path:parse(Path) of
        {ok, #{schema := Schema, module := Mod}} ->
            case parse_schema_value(Schema, Raw) of
                {error, Msg} ->
                    {error, Return, View, Mode, #{Path => Msg}, #{}};
                {ok, Val} ->
                    after_etag(Etag, Return, View, Mode, #{},
                               fun() ->
                                       Body = #{qname(Mod, maps:get(name, Schema)) => Val},
                                       case apply_save(Path, Body) of
                                           ok ->
                                               {ok, Return, View, Mode};
                                           {error, Err} ->
                                               {error, Return, View, Mode,
                                                #{Path => err_msg(Err)}, #{}}
                                       end
                               end)
            end;
    ...
    case find_node(Path, modules()) of
        ...
        Node ->
            case build_item(Node, Draft, <<"item">>) of
                ...
                {ok, Item} ->
                    after_etag(Etag, Return, View, Mode, Draft,
                               fun() ->
                                       Body = #{maps:get(<<"qname">>, Node) => [Item]},
                                       case mgmtd_restconf_data:post(Path, Body) of
                                           {ok, _} ->
                                               {ok, Return, View, Mode};
                                           {error, Err} ->
                                               {error, Return, View, Mode,
                                                #{<<"add">> => err_msg(Err)}, Draft}
                                       end
                               end)
            end
    end.
...
delete(Qs) ->
    ...
    View = qs_val(Qs, <<"view">>, <<"index">>),
    Mode = parse_mode(qs_val(Qs, <<"mode">>)),
    after_etag(qs_val(Qs, <<"etag">>), Return, View, Mode, #{},
               fun() ->
                       case mgmtd_restconf_data:delete(Path) of
                           ok ->
                               {ok, Return, View, Mode};
                           {error, Err} ->
                               {error, Return, View, Mode,
...
apply_save(Path, Body) ->
    case mgmtd_restconf_data:patch(Path, Body) of
        ok ->
            ok;
        {error, #{http := 404}} ->
            case mgmtd_restconf_data:put(Path, Body) of
                {ok, _} -> ok;
                {error, _} = Err -> Err
            end;
        {error, _} = Err ->
...
page(Kind, Path, Opts) ->
    ensure_compiled(),
    Mods = maps:get(mods, Opts, modules()),
    Mode = maps:get(mode, Opts, config),
    ...
    Vars = [{css_href, ?CSS},
            ...
            {mode, mode_bin(Mode)},
            {mode_switch_html, html(render_mode_switch(View, Path, Mods, Mode))},
            {tree_html, html(Tree)},
            {pane_html, html(Pane)}],
    tpl(page_mod(Kind), Vars).

page_mod(index) -> mgmtd_ui_index_dtl;
page_mod(content) -> mgmtd_ui_content_dtl.

page_base(index) -> ?UI_BASE;
page_base(content) -> <<"/mgmtd/ui/content">>.
...
modules() ->
    maps:get(<<"modules">>, mgmtd_ui_schema:snapshot()).
...
loc(View, Path, Mode) ->
    loc_href(page_base(view_kind(View)), Path, Mode).
...
render_pane(Node, _Path, Errors, Draft, View, Mode, _Mods) ->
    ...
    ValueHtml = case LoadErr of
                    undefined ->
                        render_value(Node, Value, Return, Return, Etag,
...
load_value(Node, Mode) ->
    Path = maps:get(<<"path">>, Node),
    case mgmtd_restconf_data:resource(Path, content_for(Node, Mode)) of
        {ok, Map} ->
            {undefined, unwrap(Map, Node)};
        {error, #{http := 404}} ->
            {undefined, empty_value(Node)};
        {error, Err} ->
            {err_msg(Err), undefined}
    end.

content_for(_Node, config) ->
    config;
content_for(#{<<"config">> := false}, oper) ->
    nonconfig;
content_for(_Node, oper) ->
    all.
...
render_value(Node, Value, ResourcePath, Return, Etag, Errors, Draft, View, Mode) ->
...
render_leaf(Node, Value, ResourcePath, Return, Etag, Errors, View, Mode) ->
    ...
    InputHtml = tpl(mgmtd_ui_input_dtl, [{input, input_vars(<<"value">>, Node, Draft)}]),
    ...
    tpl(mgmtd_ui_leaf_dtl, put_text(Vars, error, maps:get(ResourcePath, Errors, undefined))).

render_container(Node, Value, ResourcePath, Return, Etag, Errors, Draft, View, Mode) ->
    Kids = maps:get(<<"children">>, Node, []),
    Rows = [kv_row(C, child_value(Value, C),
                   child_path(ResourcePath, maps:get(<<"name">>, C)),
                   Return, Etag, Errors, Draft, View, Mode)
            || C <- Kids],
    tpl(mgmtd_ui_container_dtl,
        [{empty, Kids =:= []}, {rows_html, html(Rows)}]).

render_list(Node, Value, Return, Etag, Errors, Draft, View, Mode) ->
    Rows = as_array(Value),
    CanWrite = writable(Node, Mode),
    Items = [render_list_item(Node, Row, Return, Etag, Errors, View, Mode)
             || Row <- Rows],
    ...
    tpl(mgmtd_ui_list_dtl, put_text(Vars, add_error, maps:get(<<"add">>, Errors, undefined))).

render_list_item(Node, Row, Return, Etag, Errors, View, Mode) ->
    ...
    Fields = [kv_row(C, child_value(Row, C),
                     child_path(Inst, maps:get(<<"name">>, C)),
                     Return, Etag, Errors, #{}, View, Mode)
              || C <- ValueFields],
    Nested = [nested_block(C, child_value(Row, C),
                           child_path(Inst, maps:get(<<"name">>, C)),
                           Return, Etag, Errors, View, Mode)
    ...
    tpl(mgmtd_ui_list_item_dtl, put_text(Vars, key_legend, KeyLegend)).

kv_row(Child, Val, ResourcePath, Return, Etag, Errors, Draft, View, Mode) ->
    Kind = maps:get(<<"kind">>, Child),
    Inner = case Kind of
                <<"leaf">> ->
                    render_leaf(Child, Val, ResourcePath, Return, Etag, Errors, View, Mode);
                <<"leaf-list">> ->
                    render_leaf(Child, Val, ResourcePath, Return, Etag, Errors, View, Mode);
                _ ->
                    render_value(Child, Val, ResourcePath, Return, Etag, Errors, Draft, View, Mode)
            end,
    tpl(mgmtd_ui_kv_row_dtl,
        [{label, maps:get(<<"name">>, Child)},
         {oper, false},
         {value_html, html(Inner)}]).

nested_block(Child, Val, ResourcePath, Return, Etag, Errors, View, Mode) ->
    Inner = render_value(Child, Val, ResourcePath, Return, Etag, Errors, #{}, View, Mode),
    tpl(mgmtd_ui_nested_dtl,
...
render_draft_field(Field, Prefix, Draft, Errors) ->
    ...
    case maps:get(<<"kind">>, Field) of
        ...
        _ ->
            ...
            InputHtml = tpl(mgmtd_ui_input_dtl,
...
parse_schema_value(Schema, Raw) ->
    case maps:get(node_type, Schema) of
        leaf_list ->
            parse_typed_leaf_list(maps:get(type, Schema, string), Raw);
        leaf ->
            parse_typed_leaf(maps:get(type, Schema, string), Raw);
        _ ->
            {error, <<"not a leaf">>}
    end.
...
parse_typed_leaf_list(Type, Raw) ->
    Lines = [L || L <- binary:split(Raw, <<"\n">>, [global]),
                  string:trim(L) =/= <<>>],
    parse_typed_lines(Lines, Type, []).

parse_typed_lines([], _Type, Acc) ->
    {ok, lists:reverse(Acc)};
parse_typed_lines([L | Rest], Type, Acc) ->
    case parse_typed_leaf(Type, string:trim(L)) of
        {ok, V} -> parse_typed_lines(Rest, Type, [V | Acc]);
        {error, _} = Err -> Err
    end.

parse_typed_leaf(boolean, <<"true">>) -> {ok, true};
parse_typed_leaf(boolean, <<"false">>) -> {ok, false};
parse_typed_leaf(boolean, _) -> {error, <<"boolean must be true or false">>};
parse_typed_leaf(Type, Bin) ->
...
is_int_type(T) when T =:= uint8; T =:= uint16; T =:= uint32; T =:= uint64;
                    T =:= int8; T =:= int16; T =:= int32; T =:= int64;
                    T =:= integer; T =:= 'inet:port-number' ->
    true;
is_int_type({T, _}) ->
    is_int_type(T);
is_int_type(_) ->
...
qname(Module, Name) ->
    iolist_to_binary([to_bin(Module), $:, to_bin(Name)]).
...
ensure_compiled() ->
    case code:ensure_loaded(mgmtd_ui_content_dtl) of
        {module, _} ->
            ok;
        {error, _} ->
            case compile() of
                ok -> ok;
                {error, Reason} -> error({mgmtd_ui_templates, Reason})
            end
    end.
